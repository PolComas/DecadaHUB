drop view if exists public.v_client_kpis_30d;
drop view if exists public.v_thread_response_events;
...
select
  ...
  end as response_natural_hours,
  case
    when reply.sent_at is null then null
    else public.business_hours_between(origin.sent_at, reply.sent_at)
  end as response_business_hours
...
response_stats as (
  select
    r.client_id,
    avg(r.response_business_hours) filter (
    ...
    avg(r.response_natural_hours) filter (
      where r.response_type = 'client_response'
        and r.reply_message_id is not null
    ) as avg_client_response_natural_hours_30d,
    avg(r.response_natural_hours) filter (
      where r.response_type = 'team_response'
        and r.reply_message_id is not null
    ) as avg_team_response_natural_hours_30d
...
select
  ...
  round(coalesce(rs.avg_client_response_natural_hours_30d, 0)::numeric, 2) as avg_client_response_natural_hours_30d,
  round(coalesce(rs.avg_team_response_natural_hours_30d, 0)::numeric, 2) as avg_team_response_natural_hours_30d,
